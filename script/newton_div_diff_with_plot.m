...
valx = linspace(min(x),max(x),1000);
valy = zeros(1,size(valx,2));

for num = 1:size(valx,2)
...
grid on
hold on
title("Newton's Divided Difference Method");
xlabel("x");
ylabel("p(x), f(x)");
plot(valx, valy,'blue');
scatter(x, fx, 'filled', 'red');
legend('p(x)','f(x)','location','southeast');
hold off
