...
while strcmpi(repeat,'Y') || strcmpi(str,'y')
    ...
    plt = input("> ", 's');
    % Failsafe
    while ~strcmpi(plt,'y') && ~strcmpi(plt,'Y') && ~strcmpi(plt,'N') && ~strcmpi(plt,'n')
        plt = input("[y/n]> ", 's');
    end
    
    if strcmpi(plt,'y') || strcmpi(plt,'Y')
        xline = linspace(x(1), x(size(x,2)), 1000);
        yline = zeros(1,size(xline,2));
        
        % calculate yline
        
        for m = 1:size(xline,2)
            ...
            for i = 1:length
               ...
               for j = 1:length
                  if j ~= i
                     prodt = prodt * (xline(m) - x(j))/(x(i) - x(j));
                  end
               end
        ...
        realx = linspace(-30, 30, 1000);
        realy = (10.*cos(0.1.*realx)).*(sin(realx)+0.1.*cos(5.*realx));
        
        grid on
        hold on
        title('f(x) = [10cos(0.1x)][sin(x)+0.1cos(5x)]');
        xlabel('x');
        ylabel('f(x)');
        scatter(x, fx, 'filled', 'green');
%         scatter(xp, yp, 'filled', 'green');
        plot(xline, yline, 'blue');
        plot(realx, realy, 'red');
        legend('Given data', 'Interpolation', 'Real graph');
        hold off
